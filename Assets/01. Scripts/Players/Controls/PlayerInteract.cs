...
public class PlayerInteract : MonoBehaviour, IPlayerInteractHandler
{
    ...
    private void SetInteractableTarget()
    {
        ...
        var targets = Physics.OverlapSphere(transform.position, stat.MaxInteractDistance, interactLayer);
    ...
    private float CheckCanInteract(Collider target)
    {
        ...
        float dis = Mathf.InverseLerp(radius, stat.MaxInteractDistance, Vector3.Distance(transform.position, trgClosestPos));
        // 거리에 따른 상호작용 가능한 각도 저장
        float canInteractAngle = Mathf.Lerp(stat.MaxInteractAngle, stat.MinInteractAngle, dis);
        // 물체의 표면 좌표와의 방향 저장
