...
namespace VolumetricFogAndMist2 {
    ...
    public class PointLightManager : MonoBehaviour, IVolumetricFogManager {
        ...
        public bool enableDistanceDimmer = true;
        [Tooltip("Distance at which point lights start to dim")]
        public float distanceStartDimming = 70f;
        [Tooltip("Distance at which point lights are completely dimmed")]
        ...
        void SubmitPointLightData() {
            ...
            bool isDimming = enableDistanceDimmer;
            ...
            for (int i = 0; count < MAX_POINT_LIGHTS && i < pointLightsCount; i++) {
                ...
                if (isDimming) {
                    float dim = 1f - Mathf.Clamp01((toLightSqr - minDistSqr) / (maxDistSqr - minDistSqr));
                    multiplier *= dim;
                }
